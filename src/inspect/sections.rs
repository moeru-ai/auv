...
impl InspectSection for CorePrefixSection {
  ...
  fn collect(&self, store: &LocalStore, run: &CanonicalRun) -> Result<Option<InspectSectionOutput>, InspectError> {
    collect_core_prefix(store, run, self.id())
  }
}

fn collect_core_prefix(store: &LocalStore, run: &CanonicalRun, id: &'static str) -> Result<Option<InspectSectionOutput>, InspectError> {
