...
use crate::{TrainingResultSpatialQueryManifest, derive_action_readiness};
...
pub struct MinecraftArtifactRef {
  /// TODO: Keep this string-shaped field only until run-contract Task 22
  /// removes the synchronous product adapter that requires `as_str()`.
  pub artifact_id: String,
}

#[derive(Clone, Debug, PartialEq, serde::Serialize)]
pub struct MinecraftTrainingResultSpatialQueryManifestLineage {
  pub artifact: MinecraftArtifactRef,
  pub manifest: Option<TrainingResultSpatialQueryManifest>,
...
pub fn derive_minecraft_training_result_spatial_query_action_readiness(
  ...
  let Some(manifest) = &lineage.manifest else {
    return MinecraftTrainingResultSpatialQueryActionReadinessSummary {
      ...
      window_point: None,
      refusal_reason: None,
      issue: Some("minecraft training result spatial query manifest missing".to_string()),
    };
  };
...
/// TODO: The product query-wiring adapter still calls this synchronous
/// legacy-shaped name. Run-contract Task 22 must move that caller to a
/// `RunSnapshot` and the typed spatial-query reader before removing this stub.
#[doc(hidden)]
pub fn extract_minecraft_training_result_spatial_query_manifests<S, R>(
  _store: &S,
  _run: &R,
) -> Result<Vec<MinecraftTrainingResultSpatialQueryManifestLineage>, String> {
  Err("Minecraft canonical artifact readers require a RunStore and RunSnapshot".to_string())
}
...
/// TODO: The inspect-server extension still calls this run-ID API. Run-contract
/// Task 22 must supply a canonical snapshot and use typed readers; this task
/// intentionally provides no legacy-store fallback.
#[doc(hidden)]
pub fn quality_baseline_report_with_verdicts_for_run<S>(
  _store: &S,
  _run_id: &str,
) -> Result<MinecraftQualityBaselineReportWithVerdicts, String> {
  Err("Minecraft quality inspection requires a canonical RunSnapshot".to_string())
}
