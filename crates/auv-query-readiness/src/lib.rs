...
pub fn format_query_not_consumable_refusal(
  status_label: &str,
...
mod tests {
  ...
  fn eligibility_as_str_covers_all_labels() {
    assert_eq!(
      DerivedActionEligibility::NotConsumable.as_str(),
      "not_consumable"
    );
    assert_eq!(
      DerivedActionEligibility::AnswerNonClickable.as_str(),
  ...
  fn constructors_set_eligibility_and_refusal_reason() {
    ...
    assert_eq!(
      not_consumable.eligibility,
      DerivedActionEligibility::NotConsumable
    );
    assert_eq!(
      not_consumable.refusal_reason.as_deref(),
      Some("status=blocked")
    );

    let answer_non_clickable =
      DerivedActionReadiness::answer_non_clickable("visibility=outside_window");
    assert_eq!(
      answer_non_clickable.eligibility,
      DerivedActionEligibility::AnswerNonClickable
    );
    assert_eq!(
      answer_non_clickable.refusal_reason.as_deref(),
      Some("visibility=outside_window")
    );

    let click_ready = DerivedActionReadiness::click_ready();
    assert_eq!(
      click_ready.eligibility,
      DerivedActionEligibility::ClickReady
    );
  ...
  fn format_query_not_consumable_refusal_with_and_without_reason() {
    assert_eq!(
      format_query_not_consumable_refusal("failed", Some("target_absent")),
      "status=failed reason=target_absent"
    );
    assert_eq!(
      format_query_not_consumable_refusal("blocked", None),
      "status=blocked"
    );
  }
}
