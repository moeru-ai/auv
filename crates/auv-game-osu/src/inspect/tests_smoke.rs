...
#[test]
fn osu_sections_emit_legacy_empty_headers() {
  let root = std::env::temp_dir()
    .join(format!("auv-osu-inspect-{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos()));
  let store = LocalStore::new(root.clone()).expect("store");
  let run_id = RunId::new("run_osu_empty");
  write_minimal_run(&store, &run_id);
  let mut sections = crate::inspect_sections_primary();
  sections.extend(crate::inspect_sections_detection_eval());
