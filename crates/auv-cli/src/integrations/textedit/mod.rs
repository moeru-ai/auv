...
async fn write_document_with_driver(
  ...
  for outcome in &report.outcomes {
    if let Some(result) = &outcome.input_action_result {
      cancellation.check().map_err(|error| error.to_string())?;
      let _ = auv_runtime::run_read::publish_input_action_result(Some(&context), result).await;
    }
  }
...
mod tests {
  ...
  use auv_tracing::{AuthorityId, MemoryRunStore, RunId, RunStore, configure, dispatcher};
