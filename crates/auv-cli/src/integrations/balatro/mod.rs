...
mod tests {
  ...
  use auv_cli_invoke::{InvokeCancellation, InvokeNamespace};

  use super::*;

  #[test]
  fn blind_select_request_requires_a_typed_blind_slot() {
    let request = parse_blind_select_request(
      None,
      &BTreeMap::from([
        ("slot".to_string(), "blind:1".to_string()),
        ("confirm_started".to_string(), "false".to_string()),
      ]),
    )
    .expect("blind-select request");

    assert_eq!(request.slot, SlotId::new(ObjectZone::Blind, 1));
    assert!(!request.confirm_started);
    assert!(parse_blind_select_request(None, &BTreeMap::new()).is_err());
    assert!(parse_blind_select_request(None, &BTreeMap::from([("slot".to_string(), "hand:1".to_string())])).is_err());
  }

  #[test]
  fn blind_skip_request_uses_one_explicit_confirmation_switch() {
    let request = parse_blind_skip_request(None, &BTreeMap::new()).expect("default blind-skip request");

    assert!(request.confirm_exit);
    assert!(parse_blind_skip_request(None, &BTreeMap::from([("confirmation".to_string(), "weak".to_string())])).is_err());
    assert!(parse_blind_skip_request(None, &BTreeMap::from([("confirm_exit".to_string(), "yes".to_string())])).is_err());
  }

  #[test]
  fn cards_clear_request_accepts_only_a_bounded_timeout() {
    let request = parse_cards_clear_request(None, &BTreeMap::new()).expect("default cards-clear request");

    assert_eq!(request.target, "Balatro");
    assert_eq!(request.timeout_ms, 1500);
    assert!(parse_cards_clear_request(None, &BTreeMap::from([("verify".to_string(), "true".to_string())])).is_err());
    assert!(parse_cards_clear_request(None, &BTreeMap::from([("timeout_ms".to_string(), "0".to_string())])).is_err());
  }

  #[test]
  fn cards_select_and_commit_requests_require_unique_hand_slots() {
    let inputs = BTreeMap::from([("slots".to_string(), "hand:0,hand:2".to_string())]);
    let select = parse_cards_select_request(None, &inputs).expect("cards-select request");
    let commit = parse_card_commit_request(CARDS_PLAY_COMMAND_ID, None, &inputs).expect("cards-play request");

    assert_eq!(
      select.slots,
      vec![
        SlotId::new(ObjectZone::Hand, 0),
        SlotId::new(ObjectZone::Hand, 2)
      ]
    );
    assert_eq!(commit.slots, select.slots);
    assert!(commit.confirm_change);
    assert!(parse_cards_select_request(None, &BTreeMap::from([("slots".to_string(), "joker:0".to_string())])).is_err());
    assert!(
      parse_card_commit_request(CARDS_DISCARD_COMMAND_ID, None, &BTreeMap::from([("slots".to_string(), "hand:1,hand:1".to_string())]),)
        .is_err()
    );
  }

  #[test]
  fn game_restart_request_uses_one_started_state_switch() {
    let request = parse_game_restart_request(None, &BTreeMap::new()).expect("default game-restart request");

    assert!(request.confirm_started);
    assert_eq!(request.timeout_ms, 1800);
    assert!(parse_game_restart_request(None, &BTreeMap::from([("confirmation".to_string(), "weak".to_string())])).is_err());
    assert!(parse_game_restart_request(None, &BTreeMap::from([("confirm_started".to_string(), "yes".to_string())])).is_err());
  }

  #[test]
  fn object_sell_requests_keep_joker_and_consumable_slots_distinct() {
    let joker = parse_joker_sell_request(
      None,
      &BTreeMap::from([
        ("slot".to_string(), "joker:1".to_string()),
        ("confirm_sale".to_string(), "false".to_string()),
      ]),
    )
    .expect("joker-sell request");
    let consumable = parse_consumable_sell_request(None, &BTreeMap::from([("slot".to_string(), "consumable:2".to_string())]))
      .expect("consumable-sell request");

    assert_eq!(joker.slot, SlotId::new(ObjectZone::Joker, 1));
    assert!(!joker.confirm_sale);
    assert_eq!(consumable.slot, SlotId::new(ObjectZone::Consumable, 2));
    assert!(parse_joker_sell_request(None, &BTreeMap::from([("slot".to_string(), "consumable:1".to_string())])).is_err());
  }

  #[test]
  fn consumable_use_request_parses_typed_hand_targets() {
    let request = parse_consumable_use_request(
      None,
      &BTreeMap::from([
        ("slot".to_string(), "consumable:2".to_string()),
        ("hand_targets".to_string(), "hand:0,hand:3".to_string()),
        ("confirm_use".to_string(), "false".to_string()),
      ]),
    )
    .expect("consumable-use request");

    assert_eq!(request.slot, SlotId::new(ObjectZone::Consumable, 2));
    assert_eq!(
      request.hand_targets,
      vec![
        SlotId::new(ObjectZone::Hand, 0),
        SlotId::new(ObjectZone::Hand, 3)
      ]
    );
    assert!(!request.confirm_use);
    assert!(parse_consumable_use_request(None, &BTreeMap::from([("slot".to_string(), "joker:2".to_string())])).is_err());
    assert!(
      parse_consumable_use_request(
        None,
        &BTreeMap::from([
          ("slot".to_string(), "consumable:2".to_string()),
          ("hand_targets".to_string(), "joker:0".to_string()),
        ]),
      )
      .is_err()
    );
  ...
  #[test]
  fn cash_out_request_defaults_to_targeted_confirmation() {
    let request = parse_cash_out_request(None, &BTreeMap::new()).expect("default cash-out request");

    assert_eq!(request.target, "Balatro");
    assert_eq!(request.confirmation, CashOutConfirmationRequest::Targeted);
    assert_eq!(request.timeout_ms, 1200);
  }

  #[test]
  fn cash_out_request_rejects_unknown_inputs_and_unbounded_timeout() {
    assert!(parse_cash_out_request(None, &BTreeMap::from([("verify".to_string(), "true".to_string())])).is_err());
    assert!(parse_cash_out_request(None, &BTreeMap::from([("timeout_ms".to_string(), "0".to_string())])).is_err());
    assert!(parse_cash_out_request(None, &BTreeMap::from([("timeout_ms".to_string(), "60001".to_string())])).is_err());
  }

  #[test]
  fn pack_skip_request_defaults_to_exit_confirmation() {
    let request = parse_pack_skip_request(None, &BTreeMap::new()).expect("default pack-skip request");
  ...
  #[test]
  fn pack_skip_request_rejects_policy_names_and_invalid_booleans() {
    assert!(parse_pack_skip_request(None, &BTreeMap::from([("confirmation".to_string(), "targeted".to_string())])).is_err());
    assert!(parse_pack_skip_request(None, &BTreeMap::from([("confirm_exit".to_string(), "yes".to_string())])).is_err());
  }

  #[test]
  fn pack_choose_request_parses_choice_and_typed_hand_targets() {
    let request = parse_pack_choose_request(
      None,
      &BTreeMap::from([
        ("choice".to_string(), "pack:1".to_string()),
        ("hand_targets".to_string(), "hand:0,hand:2".to_string()),
        ("confirm_applied".to_string(), "false".to_string()),
      ]),
    )
    .expect("pack-choose request");

    assert_eq!(request.choice, PackChoiceId::new(1));
    assert_eq!(
      request.hand_targets,
      vec![
        SlotId::new(ObjectZone::Hand, 0),
    ...
    assert!(!request.confirm_applied);
    assert!(parse_pack_choose_request(None, &BTreeMap::from([("choice".to_string(), "hand:1".to_string())])).is_err());
  }

  #[test]
  fn store_next_round_request_defaults_to_targeted_confirmation() {
    let request = parse_store_next_round_request(None, &BTreeMap::new()).expect("default store next-round request");

    assert_eq!(request.target, "Balatro");
    assert_eq!(request.confirmation, StoreNextRoundConfirmationRequest::Targeted);
    assert_eq!(request.timeout_ms, 1200);
  }

  #[test]
  fn store_buy_request_requires_a_store_slot_and_one_purchase_switch() {
    let request = parse_store_buy_request(
      None,
      &BTreeMap::from([
        ("slot".to_string(), "store:2".to_string()),
        ("confirm_purchase".to_string(), "false".to_string()),
      ]),
    )
    .expect("store-buy request");

    assert_eq!(request.slot, SlotId::new(ObjectZone::Store, 2));
    assert!(!request.confirm_purchase);
    assert!(parse_store_buy_request(None, &BTreeMap::new()).is_err());
    assert!(parse_store_buy_request(None, &BTreeMap::from([("slot".to_string(), "joker:2".to_string())])).is_err());
  }

  #[test]
  fn store_next_round_request_rejects_unknown_inputs_and_unbounded_timeout() {
    assert!(parse_store_next_round_request(None, &BTreeMap::from([("verify".to_string(), "true".to_string())])).is_err());
    assert!(parse_store_next_round_request(None, &BTreeMap::from([("timeout_ms".to_string(), "0".to_string())])).is_err());
    assert!(parse_store_next_round_request(None, &BTreeMap::from([("timeout_ms".to_string(), "60001".to_string())])).is_err());
  }

  #[tokio::test]
  async fn cash_out_dry_run_never_reaches_the_live_driver() {
    let command = cash_out_invoke_command();

    let output = command
      .invoke(InvokeCommandInput {
        command_id: CASH_OUT_COMMAND_ID.to_string(),
        target_application_id: Some("Fixture Balatro".to_string()),
        inputs: BTreeMap::new(),
        dry_run: true,
        cancellation: InvokeCancellation::new(),
      })
      .await
      .expect("cash-out dry run");

    assert_eq!(command.namespace, InvokeNamespace::Game);
    assert_eq!(output, InvokeCommandOutput::completed());
  }

  #[tokio::test]
  async fn game_restart_dry_run_never_reaches_the_live_driver() {
    let output = game_restart_invoke_command()
      .invoke(InvokeCommandInput {
        command_id: GAME_RESTART_COMMAND_ID.to_string(),
      ...
      .await
      .expect("game-restart dry run");

    assert_eq!(output, InvokeCommandOutput::completed());
  }

  #[tokio::test]
  async fn object_sell_commands_dry_run_without_reaching_the_live_driver() {
    let joker = joker_sell_invoke_command()
      .invoke(InvokeCommandInput {
        command_id: JOKER_SELL_COMMAND_ID.to_string(),
        target_application_id: None,
        inputs: BTreeMap::from([("slot".to_string(), "joker:0".to_string())]),
        dry_run: true,
        cancellation: InvokeCancellation::new(),
      })
      .await
      .expect("joker-sell dry run");
    let consumable = consumable_sell_invoke_command()
      .invoke(InvokeCommandInput {
        command_id: CONSUMABLE_SELL_COMMAND_ID.to_string(),
        target_application_id: None,
        inputs: BTreeMap::from([("slot".to_string(), "consumable:0".to_string())]),
        dry_run: true,
        cancellation: InvokeCancellation::new(),
      })
      .await
      .expect("consumable-sell dry run");

    assert_eq!(joker, InvokeCommandOutput::completed());
    assert_eq!(consumable, InvokeCommandOutput::completed());
  }

  #[tokio::test]
  async fn cards_clear_dry_run_never_reaches_the_live_driver() {
    let command = cards_clear_invoke_command();

    let output = command
      .invoke(InvokeCommandInput {
        command_id: CARDS_CLEAR_COMMAND_ID.to_string(),
        target_application_id: Some("Fixture Balatro".to_string()),
        inputs: BTreeMap::new(),
        dry_run: true,
        cancellation: InvokeCancellation::new(),
  ...
  #[tokio::test]
  async fn blind_commands_dry_run_without_reaching_the_live_driver() {
    let select = blind_select_invoke_command()
      .invoke(InvokeCommandInput {
        command_id: BLIND_SELECT_COMMAND_ID.to_string(),
        target_application_id: None,
        inputs: BTreeMap::from([("slot".to_string(), "blind:1".to_string())]),
        dry_run: true,
        cancellation: InvokeCancellation::new(),
      })
      .await
      .expect("blind-select dry run");
    let skip = blind_skip_invoke_command()
      .invoke(InvokeCommandInput {
        command_id: BLIND_SKIP_COMMAND_ID.to_string(),
        target_application_id: None,
        inputs: BTreeMap::new(),
        dry_run: true,
        cancellation: InvokeCancellation::new(),
      ...
      .await
      .expect("blind-skip dry run");

    assert_eq!(select, InvokeCommandOutput::completed());
    assert_eq!(skip, InvokeCommandOutput::completed());
  }

  #[tokio::test]
  async fn pack_skip_dry_run_never_reaches_the_live_driver() {
    let command = pack_skip_invoke_command();

    let output = command
      .invoke(InvokeCommandInput {
        command_id: PACK_SKIP_COMMAND_ID.to_string(),
        target_application_id: Some("Fixture Balatro".to_string()),
        inputs: BTreeMap::new(),
        dry_run: true,
        cancellation: InvokeCancellation::new(),
  ...
  #[tokio::test]
  async fn store_next_round_dry_run_never_reaches_the_live_driver() {
    let command = store_next_round_invoke_command();

    let output = command
      .invoke(InvokeCommandInput {
        command_id: STORE_NEXT_ROUND_COMMAND_ID.to_string(),
        target_application_id: Some("Fixture Balatro".to_string()),
        inputs: BTreeMap::new(),
        dry_run: true,
        cancellation: InvokeCancellation::new(),
      ...
      .await
      .expect("store next-round dry run");
  ...
  #[tokio::test]
  async fn store_buy_dry_run_never_reaches_the_live_driver() {
    let output = store_buy_invoke_command()
      .invoke(InvokeCommandInput {
        command_id: STORE_BUY_COMMAND_ID.to_string(),
        target_application_id: None,
        inputs: BTreeMap::from([("slot".to_string(), "store:0".to_string())]),
        dry_run: true,
        cancellation: InvokeCancellation::new(),
      })
      .await
      .expect("store-buy dry run");
