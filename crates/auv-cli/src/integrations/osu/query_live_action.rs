...
use auv_runtime::contract::{ArtifactRef, FreshnessBasis, OPERATION_RESULT_API_VERSION, OperationOutput, OperationResult};
...
pub fn stage_osu_query_wired_live_action_operation_result(
  ...
  let artifact_json = serde_json::to_string_pretty(operation_result)
    ...
    .map_err(|error| format!("failed to serialize osu query wired live action operation result: {error}"))?;
  context
    .stage_artifact_bytes_with_ref(
      "operation-result",
      artifact_json.as_bytes(),
      "operation-result.json",
      Some("osu visual truth query wired live action operation result with spatial query lineage".to_string()),
    )
