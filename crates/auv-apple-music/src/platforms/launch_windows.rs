...
pub enum LaunchEvent {
  WindowResolved,
  WindowNotFound,
  LaunchTargetsDiscovered { app_ids: Vec<String> },
  LaunchTargetsMissing,
  LaunchTargetDiscoveryFailed { message: String },
  LaunchSucceeded { app_id: String },
  LaunchFailed { app_id: String, message: String },
  WindowAppeared,
  WindowWaitTimedOut { timeout_ms: u64 },
  UnsupportedPlatform,
}
...
pub fn run_open_window(inputs: &OpenWindowInputs) -> Result<LaunchResult, String> {
  crate::tracing::window_open(|| run_open_window_inner(inputs))
}

fn run_open_window_inner(inputs: &OpenWindowInputs) -> Result<LaunchResult, String> {
  let mut result = LaunchResult::new();

  // Step 1: resolve existing window.
  match resolve_window(&inputs.resolve)? {
    Some(window) => {
      result.events.push(LaunchEvent::WindowResolved);
      result.record_window(window);
      return Ok(result);
    }
    None => {
      result.events.push(LaunchEvent::WindowNotFound);
    }
  }
  ...
  #[cfg(target_os = "windows")]
  {
    ...
    match &discovered {
      Ok(ids) if !ids.is_empty() => result.events.push(LaunchEvent::LaunchTargetsDiscovered {
        app_ids: ids.clone(),
      }),
      Ok(_) => result.events.push(LaunchEvent::LaunchTargetsMissing),
      Err(message) => result.events.push(LaunchEvent::LaunchTargetDiscoveryFailed {
    ...
    for app_id in app_user_model_id_candidates(discovered.unwrap_or_default()) {
      match launch_via_shell_uri(&app_id) {
        Ok(()) => {
          result.events.push(LaunchEvent::LaunchSucceeded { app_id });
          break;
        }
        Err(message) => {
          result.events.push(LaunchEvent::LaunchFailed { app_id, message });
        }
      }
    }
    ...
    if inputs.settle_ms > 0 {
      ...
      loop {
        match resolve_window(&inputs.resolve)? {
          Some(window) => {
            result.events.push(LaunchEvent::WindowAppeared);
            result.record_window(window);
            return Ok(result);
          }
          None => {}
      ...
      result.events.push(LaunchEvent::WindowWaitTimedOut {
  ...
  {
    result.events.push(LaunchEvent::UnsupportedPlatform);
  }
