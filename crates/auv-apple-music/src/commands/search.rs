...
pub fn run_search_result_select(
  inputs: &SearchResultSelectInputs,
) -> Result<SearchResultSelection, String> {
  ...
  let (selected, selection_input, verification) = driver.select_result(
    anchor,
    Duration::from_millis(inputs.selection_timeout_ms),
    Duration::from_millis(inputs.search.settle_ms),
  )?;
...
trait SearchDriver {
  fn prepare_window(
    ...
    settle: Duration,
  ) -> Result<(Option<String>, WindowMutationResult), String>;
...
fn run_search_with_driver(
  inputs: &SearchInputs,
  driver: &mut impl SearchDriver,
  ...
  let verification = driver.verify_query(
    query,
    Duration::from_millis(inputs.verification_timeout_ms),
    inputs.artifact_dir.as_deref(),
  )?;

  let diagnostics = if verification.status == SearchVerificationStatus::Unverified {
    vec![
      "query input was delivered, but neither UI Automation nor fallback OCR verified the query"
        .to_string(),
    ]
  } else {
    Vec::new()
...
mod platform {
  ...
  impl super::SearchDriver for UnsupportedSearchDriver {
    ...
    fn focus_search(
      &mut self,
      _settle: std::time::Duration,
    ) -> Result<auv_driver::input::InputActionResult, String> {
      unreachable!("non-Windows search driver cannot be opened")
    }
    ...
    fn select_result(
      ...
      (
        super::SearchResultMatch,
        auv_driver::input::InputActionResult,
        super::SearchVerification,
      ),
      String,
    > {
      unreachable!("non-Windows search driver cannot be opened")
...
mod platform {
  ...
  use super::{
    SearchDriver, SearchResultMatch, SearchVerification, SearchVerificationStatus, normalized,
    resolve_result_match, search_input_path, selection_navigation_evidence, uia_query_evidence,
  };
  ...
  pub(super) fn open_driver() -> Result<WindowsSearchDriver, String> {
    let session = WindowsDriver::new()
      .open_local()
      .map_err(|error| format!("windows driver open failed: {error}"))?;
  ...
  impl SearchDriver for WindowsSearchDriver {
    fn prepare_window(
      ...
      let window = resolve_window(resolve)?
    ...
    fn focus_search(
      ...
      let window = self
        .window
        .as_ref()
        .ok_or_else(|| "search focus requires a prepared window".to_string())?;
      let snapshot = self
        .session
        .accessibility()
        .snapshot_window(window)
        .map_err(|error| format!("search input UIA snapshot failed: {error}"))?;
      ...
      .ok_or_else(|| {
        "Apple Music UIA search input was not found (expected edit/TextBox node)".to_string()
      })?;
      let result = self
        .session
        .accessibility()
        .focus_node(window, &path)
        .map_err(|error| format!("search input UIA focus failed: {error}"))?;
      if !settle.is_zero() {
    ...
    fn verify_query(
      ...
      let window = self
        .window
        .as_ref()
        .ok_or_else(|| "search verification requires a prepared window".to_string())?;
      ...
      let capture = self
        ...
        .window()
        .capture(window)
        .map_err(|error| format!("search fallback capture failed: {error}"))?;
      ...
      let artifact = artifact_dir
        .map(|dir| save_artifact(dir, &capture))
        .transpose()?;
    ...
    > {
      let window = self
        .window
        .as_ref()
        .ok_or_else(|| "result selection requires a prepared window".to_string())?;
      let deadline = Instant::now() + timeout;
      ...
      loop {
        let snapshot = self
          ...
          .accessibility()
          .snapshot_window(window)
          .map_err(|error| format!("result selection UIA snapshot failed: {error}"))?;
        ...
        if Instant::now() >= deadline || timeout.is_zero() {
          return Err(format!(
            "Apple Music search result matching {anchor:?} was not found"
          ));
        }
        std::thread::sleep(Duration::from_millis(100));
      }
  ...
  fn save_artifact(
    ...
    std::fs::create_dir_all(dir)
      .map_err(|error| format!("create search artifact directory failed: {error}"))?;
    ...
    capture
      .image
      .save(&path)
      .map_err(|error| format!("save search artifact failed: {error}"))?;
...
fn search_input_path<'a>(
  nodes: impl Iterator<Item = (&'a str, &'a str, &'a str, &'a str, &'a str)>,
) -> Option<String> {
  nodes
    .filter(|(_path, name, automation_id, class_name, control_type)| {
      let is_text_box =
        automation_id.eq_ignore_ascii_case("TextBox") && class_name.eq_ignore_ascii_case("TextBox");
      let is_search_edit = control_type.eq_ignore_ascii_case("edit")
        && (name.eq_ignore_ascii_case("Search") || automation_id.eq_ignore_ascii_case("TextBox"));
  ...
  match candidates.as_slice() {
    ...
    matches => {
      let names = matches
        ...
        .join("; ");
      Err(format!(
        "search result anchor is ambiguous; {} UIA results matched: {names}",
        matches.len()
      ))
    }
  }
...
fn selection_navigation_evidence<'a>(
  selected_name: &str,
  ...
  let observed = nodes
    .iter()
    .map(|(name, _control_type, _class_name)| *name)
    .filter(|name| !name.trim().is_empty())
    .collect::<Vec<_>>()
    .join(" ");
  let observed_normalized = normalized(&observed);
  let matched_terms = identity_terms(selected_name)
    .iter()
    .filter(|term| observed_normalized.contains(term.as_str()))
    .cloned()
    .collect::<Vec<_>>();
  (matched_terms.len() >= 2).then(|| {
    format!(
      "detail view matched identity terms: {}",
      matched_terms.join(", ")
    )
  })
}
...
fn uia_query_evidence<'a>(
  query: &str,
  nodes: impl Iterator<Item = (&'a str, Option<&'a str>, &'a str, bool)>,
...
mod tests {
  ...
  use auv_driver::window::{
    WindowMutationAttempt, WindowMutationPath, WindowMutationResult, WindowState,
  };
  ...
  impl SearchDriver for RecordingDriver {
    ...
    fn submit_query(&mut self, query: &str, settle: Duration) -> Result<InputActionResult, String> {
      self
        .calls
        .push(format!("query:{query}:{}", settle.as_millis()));
  ...
  fn search_runs_typed_window_input_and_verification_sequence() {
    ...
    assert_eq!(
      result.search_focus_input.selected_path,
  ...
  fn search_keeps_unverified_delivery_inspectable() {
    ...
    assert_eq!(
      result.query_input.selected_path,
  ...
  fn search_input_path_does_not_match_unrelated_text() {
    let path =
      search_input_path([("0/2/0/0/2/5/1/0", "Current Song", "", "TextBlock", "text")].into_iter());
  ...
  fn result_match_rejects_ambiguous_anchor() {
    let error = resolve_result_match(
      ...
      [
        ...
        (
          "0/2",
          "Chopin: Ballade No. 1 (Piano) Healing Energy",
          "list item",
          "GridViewItem",
        ),
      ]
  ...
  fn selection_navigation_verifies_detail_view_from_live_shape() {
    let evidence = selection_navigation_evidence(
      ...
      [
        (
          "Chopin: Ballades, Berceuse & Mazurkas YUNDI Classical 2016",
          "group",
  ...
  fn search_result_selection_runs_search_then_typed_selection() {
    let inputs = SearchResultSelectInputs::with_query_and_anchor(
      "Chopin ballade no. 1",
      "Ballade No. 1 in G Minor, Op. 23 YUNDI",
    );
    let mut driver = RecordingDriver::new(SearchVerificationStatus::Verified);
    ...
    let search =
      run_search_with_driver(&inputs.search, &mut driver).expect("search should complete first");
    ...
    assert_eq!(
      driver.calls.last().map(String::as_str),
      Some("select:Ballade No. 1 in G Minor, Op. 23 YUNDI:5000:300")
    );
  }
  ...
  fn uia_verification_prefers_value_pattern() {
    ...
    assert_eq!(
      evidence,
      Some((
        "ui_automation_value".to_string(),
        "AURORA Cure For Me".to_string(),
      ))
    );
  }
  ...
  fn uia_verification_uses_accessible_name_when_value_is_absent() {
    let evidence = uia_query_evidence(
      "Cure For Me",
      [("AURORA — Cure For Me", None, "text", false)].into_iter(),
    );

    assert_eq!(
      evidence,
      Some((
        "ui_automation_name".to_string(),
        "AURORA — Cure For Me".to_string(),
      ))
    );
  }
  ...
  fn uia_verification_returns_none_without_query_evidence() {
    let evidence = uia_query_evidence(
      "Cure For Me",
      [("Search", Some("Different Song"), "edit", true)].into_iter(),
    );
