use crate::auv::api::core::v1::device_service_client::DeviceServiceClient;
use crate::auv::api::core::v1::discovery_service_client::DiscoveryServiceClient;
use crate::auv::api::core::v1::run_service_client::RunServiceClient;
use crate::auv::api::core::v1::runner_class_service_client::RunnerClassServiceClient;
use crate::auv::api::core::v1::runner_service_client::RunnerServiceClient;
...
use crate::auv::api::runner::v1::runner_runtime_service_client::RunnerRuntimeServiceClient;
...
#[test]
fn runner_runtime_service_has_typed_status_watch_and_no_conditions() {
  let descriptor_set = FileDescriptorSet::decode(FILE_DESCRIPTOR_SET).expect("decode FILE_DESCRIPTOR_SET");
  let file = descriptor_set
    .file
    .iter()
    .find(|file| file.name.as_deref() == Some("auv/api/runner/v1/runtime.proto"))
    .expect("Runner runtime descriptor");
  let service = file.service.iter().find(|service| service.name.as_deref() == Some("RunnerRuntimeService")).expect("runtime service");
  assert_eq!(
    service.method.iter().filter_map(|method| method.name.as_deref()).collect::<Vec<_>>(),
    ["GetMetadata", "GetStatus", "WatchStatus", "Drain"]
  );
  assert!(
    service.method.iter().find(|method| method.name.as_deref() == Some("WatchStatus")).is_some_and(|method| method.server_streaming())
  );
  assert!(service.method.iter().filter(|method| method.name.as_deref() != Some("WatchStatus")).all(|method| !method.server_streaming()));

  let status = file.message_type.iter().find(|message| message.name.as_deref() == Some("RunnerRuntimeStatus")).expect("runtime status");
  assert_eq!(status.field.iter().filter_map(|field| field.name.as_deref()).collect::<Vec<_>>(), ["phase", "operations", "observed_at"]);
  assert!(!status.field.iter().any(|field| field.name.as_deref() == Some("conditions")));
  let operations = file
    .message_type
    .iter()
    .find(|message| message.name.as_deref() == Some("RunnerRuntimeOperationsStatus"))
    .expect("runtime operations status");
  assert_eq!(operations.field.iter().filter_map(|field| field.name.as_deref()).collect::<Vec<_>>(), ["active", "queued", "capacity"]);
  assert!(operations.field.iter().all(|field| field.r#type == Some(prost_types::field_descriptor_proto::Type::Uint32 as i32)));

  fn assert_client<T>() {}
  assert_client::<RunnerRuntimeServiceClient<tonic::transport::Channel>>();
}
...
fn service_descriptor_closure_excludes_unserved_auv_services() {
  ...
  assert!(!services.contains(&"auv.api.core.v1.DeviceService".to_string()));
}
...
fn macos_media_control_service_has_typed_read_and_input_methods() {
  ...
  for (index, method) in methods.iter().enumerate() {
    let lease = method.input().get_field_by_name("lease").expect("Runner lease field");
    let prost_reflect::Kind::Message(lease_type) = lease.kind() else {
      panic!("lease must be a message");
    };
    assert_eq!(lease_type.full_name(), "auv.api.core.v1.RunnerLeaseRef");
    let options = method.options();
    assert_eq!(options.get_extension(&discoverable).as_ref(), &Value::Bool(true));
...
fn overlay_service_is_two_typed_mutations() {
  ...
  for method in methods {
    ...
    assert_eq!(
      method.input().get_field_by_name("lease").expect("lease").kind().as_message().expect("message").full_name(),
      "auv.api.core.v1.RunnerLeaseRef"
    );
  }
  fn assert_client<T>() {}
...
#[test]
fn object_detection_service_is_typed_and_lease_scoped() {
  let descriptor_set = FileDescriptorSet::decode(FILE_DESCRIPTOR_SET).expect("decode FILE_DESCRIPTOR_SET");
  ...
  let lease = request.field.iter().find(|field| field.name.as_deref() == Some("lease")).expect("lease field");
  assert_eq!(lease.type_name.as_deref(), Some(".auv.api.core.v1.RunnerLeaseRef"));
...
fn core_control_services_are_typed_and_do_not_claim_watch() {
  ...
  let mut services = descriptor_set
    .file
    .iter()
    .filter(|file| file.package.as_deref() == Some("auv.api.core.v1"))
  ...
  let runner_file =
    descriptor_set.file.iter().find(|file| file.name.as_deref() == Some("auv/api/core/v1/runner.proto")).expect("Runner descriptor");
  ...
  let capability =
    runner_file.message_type.iter().find(|message| message.name.as_deref() == Some("RunnerCapability")).expect("RunnerCapability message");
  assert_eq!(capability.field.iter().filter_map(|field| field.name.as_deref()).collect::<Vec<_>>(), vec!["service", "methods"]);
...
#[test]
fn driver_display_service_is_typed_and_runner_scoped() {
  let descriptor_set = FileDescriptorSet::decode(FILE_DESCRIPTOR_SET).expect("decode FILE_DESCRIPTOR_SET");
  let file = descriptor_set
    .file
    .iter()
  ...
  let lease = request.field.iter().find(|field| field.name.as_deref() == Some("lease")).expect("Runner lease ref");
  assert_eq!(lease.type_name.as_deref(), Some(".auv.api.core.v1.RunnerLeaseRef"));
...
#[test]
fn driver_capture_service_is_typed_and_runner_scoped() {
  let descriptor_set = FileDescriptorSet::decode(FILE_DESCRIPTOR_SET).expect("decode FILE_DESCRIPTOR_SET");
  let file = descriptor_set
    .file
    .iter()
  ...
  let display_lease = display_request.field.iter().find(|field| field.name.as_deref() == Some("lease")).expect("Runner lease ref");
  assert_eq!(display_lease.type_name.as_deref(), Some(".auv.api.core.v1.RunnerLeaseRef"));
...
#[test]
fn driver_text_recognition_service_is_typed_and_runner_scoped() {
  let descriptor_set = FileDescriptorSet::decode(FILE_DESCRIPTOR_SET).expect("decode FILE_DESCRIPTOR_SET");
  let file = descriptor_set
    .file
    .iter()
...
#[test]
fn driver_input_service_is_typed_runner_scoped_and_returns_delivery_evidence() {
  let descriptor_set = FileDescriptorSet::decode(FILE_DESCRIPTOR_SET).expect("decode FILE_DESCRIPTOR_SET");
  let file =
...
fn macos_application_service_preserves_typed_activation_verification() {
  ...
  assert_eq!(
    request.field.iter().find(|field| field.name.as_deref() == Some("lease")).and_then(|field| field.type_name.as_deref()),
    Some(".auv.api.core.v1.RunnerLeaseRef")
  );
  assert_eq!(
