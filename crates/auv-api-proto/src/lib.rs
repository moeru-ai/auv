...
/// Maximum encoded gRPC message size for Driver methods that carry one raw
/// RGBA frame.
///
/// NOTICE: tonic's 4 MiB default cannot represent a native Retina/8K capture.
/// This explicit 160 MiB bound covers one 8K RGBA8 frame plus protobuf
/// overhead while still rejecting unbounded allocations. Streaming video is a
/// separate protocol and must not raise this unary limit.
pub const MAX_CAPTURE_GRPC_MESSAGE_BYTES: usize = 160 * 1024 * 1024;
...
pub mod auv {
  pub mod api {
    ...
    pub mod core {
      pub mod v1 {
        tonic::include_proto!("auv.api.core.v1");
      }
    }
    ...
    pub mod runner {
      pub mod v1 {
        tonic::include_proto!("auv.api.runner.v1");
      }
    }
