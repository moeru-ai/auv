// Scratch: enumerate ALL created playlists via AX by scrolling + position keying.
//
// NOTICE: exploration-only, deletable. The sidebar is virtualized, so one capture
// only yields the visible rows. This scrolls top-to-bottom and accumulates rows
// keyed by their scroll-invariant offset from the section header (row.y -
// header.y), NOT by label: NetEase allows playlists with the same name (and AX
// truncates the visible name), so a label key drops the duplicates; the offset is
// the row's real position and keeps them. Read-only except for scrolling.
//
// Usage: cargo run -p auv-netease-music --example netease_ax_enumerate_all -- \
//   [bundle-id] [max_rounds] [down_delta]
...
fn main() {
  ...
  set_app_enhanced_user_interface(probe.pid as i32, true).expect("enable");
  std::thread::sleep(Duration::from_millis(900));

  // Parse the trailing count from a header label like "创建的歌单 201".
  let trailing_count =
    |label: &str| -> Option<usize> { label.split_whitespace().last().and_then(|t| t.parse().ok()) };

  // Seek to the top of the sidebar so the created list starts from item 1.
  for _ in 0..25 {
    scroll(900.0);
  }
  ...
  for round in 0..max_rounds {
    ...
    let appended = rows.len() - before;
    eprintln!(
      "round {round:>2}: +{appended} (rows {}/{:?}) visible_in_capture={captured_count}",
      rows.len(),
      created_total
    );
    dry_rounds = if appended == 0 { dry_rounds + 1 } else { 0 };
  ...
  let _ = set_app_enhanced_user_interface(probe.pid as i32, false);

  let distinct = rows.values().collect::<HashSet<_>>().len();
  println!(
    "== created playlists: {} rows ({distinct} distinct) / {} declared ==",
    rows.len(),
    created_total.map(|t| t.to_string()).unwrap_or("?".into())
  );
...
fn main() {
  eprintln!("netease_ax_enumerate_all is macOS-only");
}
