...
use crate::view_parsers::sidebar::target_probe::{SidebarTargetMissReason, analyze_sidebar_target_probe};
...
fn analyze_sidebar_target_probe_finds_playlist_item() {
  ...
  let observation = parse_sidebar_viewport(0, ViewBounds::new(0.0, 469.8, 320.0, 338.2), &recognition);
...
fn parse_viewport_classifies_sections_and_playlist_items() {
  ...
  assert_eq!(observation.candidates[1].label, Some("创建的歌单".to_string()));
  assert_eq!(observation.candidates[2].kind, SidebarCandidateKind::PlaylistItem);
  assert_eq!(observation.candidates[2].label, Some("Coding BGM".to_string()));
...
fn parse_viewport_ignores_bottom_player_bar_outside_sidebar_bounds() {
  ...
  let observation = parse_sidebar_viewport(0, ViewBounds::new(0.0, 443.0, 344.0, 528.0), &recognition);

  assert!(observation.candidates.iter().any(|candidate| candidate.label.as_deref() == Some("Coding BGM")));
  assert!(observation.candidates.iter().all(|candidate| candidate.label.as_deref() != Some("Reverberation")));
  assert!(observation.candidates.iter().all(|candidate| candidate.label.as_deref() != Some("1w+")));
}
...
fn parse_viewport_assigns_unique_candidate_ids_for_duplicate_cjk_labels() {
  ...
  let observation = parse_sidebar_viewport(0, ViewBounds::new(0.0, 0.0, 240.0, 400.0), &recognition);
  let candidate_ids = observation.candidates.iter().map(|candidate| candidate.id.as_str()).collect::<Vec<_>>();
  let unique_candidate_ids = candidate_ids.iter().copied().collect::<std::collections::HashSet<_>>();
...
fn parse_single_digit_playlist_at_sidebar_x_becomes_candidate() {
  // obs-0005 live sample (A6c-8): OCR recognized "1" at sidebar playlist row x=71.
  let recognition = fake_recognition(vec![("1", 71.0, 591.0, 10.0, 19.0)]);
  let observation = parse_sidebar_viewport(0, ViewBounds::new(0.0, 469.8, 320.16, 338.2), &recognition);

  assert_eq!(observation.evidence_nodes.len(), 1);
  assert_eq!(observation.candidates.len(), 1);
  assert_eq!(observation.candidates[0].kind, SidebarCandidateKind::PlaylistItem);
...
fn parse_viewport_treats_playlist_named_rows_as_items_not_sections() {
  ...
  let observation = parse_sidebar_viewport(0, ViewBounds::new(0.0, 0.0, 280.0, 400.0), &recognition);
