...
2. ~~**S3 durable `scan-coverage-v0`**~~ — **S8a:** `scan-coverage-v0` wire/IO cluster has `landed proof` evidence; **S8b:** scene_state consumer helper proof for coverage-derived fields — see [S8b handoff](2026-07-08-auv-scan-s8b-scene-coverage-consumer-handoff.md); **S3 substrate stage remains `partial`** until S8c runtime producer — see [S8a handoff](2026-07-07-auv-scan-s8a-coverage-wire-handoff.md).
