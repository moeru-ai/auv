...
**Scope:** `playlist` only. `song play` is a separate plan (it is welded to `auv-cli`'s run-recording machinery and needs a layering decision first). `playlist play` is deferred (net-new automation). See `docs/ai/references/2026-05-29-netease-music-cli-design.md`.
