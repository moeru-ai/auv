...
The driver synthesizes no keyboard-down event, and no keyboard hold survives the call.

Windows foreground clicks use one `SendInput` batch per click: modifier downs,
mouse down/up, and modifier ups in reverse order. The driver does not inject or release already-held modifiers.
If delivery stops after a prefix, cleanup releases the remaining injected keys and mouse button.
The driver reports delivery/release failure.

Windows background messages carry Shift/Control in `wParam` on down and up.
The driver rejects Alt/Meta for both background policies before target activation.
No implicit focus fallback occurs. Meta maps to the Windows key.

Use `ForegroundPreferred` for those modifiers.

Linux RemoteDesktop Portal clicks hold modifiers around the click sequence.
The driver attempts every release in reverse order, even after a failed press reply or click. The driver reports release failures, and the owning API drops the failed portal session.
Meta maps to Super. Background-only window delivery remains unsupported.

Portal input cannot query the user's existing key state. This is not an isolation guarantee from concurrently held physical modifiers.
...
Accepted names are `shift`, `control`/`ctrl`, `alt`/`option`,
and `meta`/`cmd`/`command`. Unknown names, empty lists and duplicate aliases are
errors. Typed invoke input retains the argument for recording/replay.
...
The JS screen helper now accepts `ScreenClickOptions`. It matches the window
helper configuration shape:
...
`clickScreenPoint(point, { click: { count: 1 } })`.

For Rust screen calls, add an explicit modifier argument (`Default::default()` for existing ordinary clicks).

The existing Buf template produces the ignored SDK files.
...
window and screen clicks. Windows tests cover event ordering, partial-delivery cleanup, and background
flag mapping. Linux tests cover platform keys and modifier cleanup after press,
...
receipt and does not establish application semantics.

Run it in a logged-in macOS GUI with Accessibility permission and window-list access:
...
ordinary click logged `0`. Request flags did not carry over.
...
The duplicated AppKit events on ChromiumCompatible form a BG-2 follow-up candidate.
Dual posting delivered duplicate events to this receiver. These duplicates affect callers that expect a single activation. This slice
...
- **Linux live input blocked:** the existing Sway session has a live
  ...
  The review changed no system packages, services, or Portal selection.
  Further input evidence requires a desktop session whose Portal implements RemoteDesktop.
...
- The Windows test library encounters two existing failures on macOS.
  `rejects_buffer_with_mismatched_length` expects the Windows OCR validator instead of the non-Windows stub.
  `window_click_background_only_fails_for_invalid_window_handle` expects `ScreenToClient` instead of the non-Windows unsupported error.

On Windows, run the ordinary native tests. Then run the opt-in receiver test:
...
messages, and records Shift/Control on both down/up followed by a plain click.
It does not claim foreground Alt/Meta receipt. Foreground input tests need the
logged-in interactive desktop. An SSH service session can lack that access.
