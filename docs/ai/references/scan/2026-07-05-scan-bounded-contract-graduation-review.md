...
[`2026-07-04-auv-s-line-graduation-review.md`](2026-07-04-scan-graduation-review.md).
...
| Parent label ([2026-07-04](2026-07-04-scan-graduation-review.md)) | This review |
| --- | --- |
...
| **Timeline** | `timeline.rs`, `motion.rs` (input) | Yes — `scan-timeline-v0` | Filter → `graduate bounded` (N-1 adjacent per [S9a](2026-07-10-scan-timeline-tracks-reference.md)) |
| **Tracks** | `tracks.rs`, `association.rs` (input) | Yes — `scan-tracks-v0` | Filter → `graduate bounded` (N-1 adjacent per [S9b](2026-07-10-scan-timeline-tracks-reference.md)) |
...
| `DIAG_INSUFFICIENT_FRAMES`, `DIAG_UNSUPPORTED_FRAME_COUNT` | Bounded diagnostic codes (`unsupported_frame_count` deprecated-by-production per [S9a](2026-07-10-scan-timeline-tracks-reference.md); builder emits `insufficient_frames` only when `< 2` frames) |
...
| **Timeline** | **`graduate bounded`** | `scan-timeline-v0` wire + IO; N-1 adjacent segments when `>= 2` frames ([S9a](2026-07-10-scan-timeline-tracks-reference.md)); diagnostic codes above | `window_bounds` delta as visual-motion proof; not a run-level substrate envelope — parent S2 motion row stays **`helper proof only`** |
| **Tracks** | **`graduate bounded`** | `scan-tracks-v0` wire + IO; N-1 adjacent association segments when `>= 2` frames + aligned observations ([S9b](2026-07-10-scan-timeline-tracks-reference.md)); two diagnostic layers; `track_id` segment-local only | Track rollup; scene_state consumer; identity continuity verdict; not a run-level substrate envelope — parent S2 tracks row stays **`hold`** |
...
[parent review](2026-07-04-scan-graduation-review.md#falsifiers) — cross-reference only.
...
- [ ] Pick zero or more follow-ups from parent [recommended next slices](2026-07-04-scan-graduation-review.md#recommended-next-slices-owner-approved-only) (wire-gap NOTICE, S3 coverage, runtime producer, N-frame timeline, B-line S6b+)
...
- [`2026-07-02-auv-scan-s1-slice1-frame-contract-handoff.md`](2026-07-03-scan-temporal-core-landed.md)
- [`2026-07-03-auv-scan-s1-s4b-motion-timeline-handoff.md`](2026-07-03-scan-temporal-core-landed.md)
- [`2026-07-03-auv-scan-s6b-scene-state-run-read-handoff.md`](2026-07-03-scan-scene-state-reference.md)
