...
**Prior work:** [A1 design charter](2026-06-30-scenebridge-design-charter.md)
...
| --- | --- | --- |
| Scene identity = ViewAnchor + ViewMemory scope | Cross-run scene target keys use `app_bundle_id`, `scope_id` / `projection_id`, and `anchor_id` from view-parser IR — not a new `SceneTarget` in `contract.rs` | [view-memory-v0](../view-memory/2026-05-29-view-parser-view-memory-v0.md), [`MatchRef`](../../../../crates/auv-netease-music/src/output.rs), [`PlaylistSidebarScan` doc](../../../../crates/auv-netease-music/src/lib.rs) L317–325 |
| Implementation locus `auv-view::memory` | `ViewMemory` writer/reader lives in existing `auv-view` crate per placement spec | [view-memory-v0](../view-memory/2026-05-29-view-parser-view-memory-v0.md) L60–62; **zero Rust** in `auv-view::memory` today |
| Surface-analyze single gate | View-derived actionable targets promote only through surface-analyze → `contract::Candidate` | [contract-bridge-v0](../view-memory/2026-05-29-view-parser-contract-bridge-v0.md) L70–75 |
| Product CLI binding first | `playlist ls --json` exposes `MatchRef`; `playlist play --candidate-id` consumes `candidate_id`; `playlist select <label>` uses keyword query — root `src/catalog.rs` unchanged | [`cli.rs`](../../../../crates/auv-netease-music/src/cli.rs), [`playlist.rs` NOTICE](../../../../crates/auv-netease-music/src/commands/playlist.rs) L353–357 |
| Hermetic proof only in A2 | Evidence pack is curated spec + synthetic fixtures; live desktop proof deferred to A3 | [A2 evidence pack](2026-06-30-scenebridge-netease-sidebar-evidence-pack.md) |
...
| Item | Status |
| --- | --- |
| [view-memory-v0](../view-memory/2026-05-29-view-parser-view-memory-v0.md) | Full persistence shape, keyed by `app_bundle_id` + `scope_id` |
| `auv-view::memory` Rust | **Not implemented** |
| [spec-vs-pr9 triage §10](../view-memory/2026-05-29-view-parser-spec-vs-pr9-divergence-triage.md) | ViewMemory + reacquire spec'd; PR-era single-shot scan only |

### Promotion and contract discipline

| Rule | Source |
| --- | --- |
| One `ArtifactRef` schema | [contract-bridge-v0](../view-memory/2026-05-29-view-parser-contract-bridge-v0.md) |
...
| Session API / MCP invoke | **separate lane** — [P14 pause](../session-api/2026-06-30-session-api-closeout.md) |
...
**Reviewer recommendation:** Package A. [view-memory-v0](../view-memory/2026-05-29-view-parser-view-memory-v0.md)
...
**Reviewer recommendation:** Package A. [contract-bridge-v0](../view-memory/2026-05-29-view-parser-contract-bridge-v0.md)
...
1. **SceneBridge ≠ session API** — grounding lane is read-side; [P14](../session-api/2026-06-30-session-api-closeout.md)
...
- Root `src/catalog.rs` `command_id` entries for NetEase
...
| --- | --- | --- |
| Owner names **SceneBridge A3** prototype | Minimal `auv-view::memory` + NetEase `playlist select` reacquire replacing rescan NOTICE | **Landed 2026-06-30** — [A3 boundary](2026-06-30-scenebridge-prototype-boundary-review.md) |
...
- [A3 prototype boundary review](2026-06-30-scenebridge-prototype-boundary-review.md) — **Package A3-min accepted**
...
- **AIRI** may donate driver primitives only — [reuse note](../ops/2026-05-13-airi-desktop-reuse.md).
...
tests pass; `auv-view::memory` implementation follows [A3 handoff](2026-06-30-scenebridge-closure.md).
...
- [view-parser merge fixtures v0](../view-memory/2026-05-29-view-parser-merge-fixtures-v0.md)
- [P14 API line pause](../session-api/2026-06-30-session-api-closeout.md)
- [AIRI desktop reuse](../ops/2026-05-13-airi-desktop-reuse.md)
