# 2026 06 30 Session Api Closeout (archived)
...
Current ownership update (2026-07-31): the session server now lives in
`crates/auv-api-server`, its Rust client in `crates/auv-api-client`, and MCP in
`crates/auv-cli`; there is no `auv-runtime` package. See the current
[`daemon and session API architecture`](2026-07-31-daemon-session-api-architecture.md).
