...
| App-local recorded operation: TextEdit document write | none | `not claimed` | `not claimed` | `not claimed` | [current core invoke registry](../crates/auv-cli-invoke/src/registry.rs) | The audited `main` does not include this command, so it receives no current public level. Pending [PR #102](https://github.com/moeru-ai/auv/pull/102) is not matrix evidence; its fixture path can establish at most `tested`, and its macOS target-window-discovery closure is blocked. |
