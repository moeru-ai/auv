...
| Screenshot and capture primitives | registry commands such as `display.capture` and `window.capture` | `tested` | `tested` | `not claimed` | [macOS capture tests](../crates/auv-driver-macos/src/session.rs), [Windows display capture invoke tests](../crates/auv-cli-invoke/src/commands/display_test.rs), [Windows window selector test](../crates/auv-cli-invoke/src/commands/window_test.rs), [Windows capture driver tests](../crates/auv-driver-windows/src/capture_test.rs), [Windows Runner IPC test](../crates/auv-cli/tests/root_cli.rs), [capture contract](TERMS_AND_CONCEPTS.md#capture-contract) | Windows evidence is fixture- and Runner IPC-oriented; PrintWindow black-frame risk remains documented in invoke paths. No current Linux desktop closure is recorded. |
| OCR and recognition primitives | registry commands such as `screen.findText` | `tested` | `tested` | `not claimed` | [macOS OCR decoder tests](../crates/auv-driver-macos/src/native/ocr.rs), [invoke screen OCR tests](../crates/auv-cli-invoke/src/commands/screen_test.rs), [invoke window OCR tests](../crates/auv-cli-invoke/src/commands/window_test.rs), [Windows OCR driver tests](../crates/auv-driver-windows/src/ocr_test.rs), [Windows window OCR session tests](../crates/auv-driver-windows/src/session_test.rs), [core registry test](../crates/auv-cli-invoke/src/lib.rs) | OCR engine availability and output quality remain target-environment dependent. |
...
| Keyboard and pointer delivery | registry `input.*` commands | `tested` | `tested` | `not claimed` | [macOS input-policy tests](../crates/auv-driver-macos/src/session.rs), [Windows keyboard invoke tests](../crates/auv-cli-invoke/src/commands/input_test.rs), [Windows input driver tests](../crates/auv-driver-windows/src/input_test.rs), [CLI coordinate dry-run tests](../crates/auv-cli/tests/root_cli.rs), [core registry test](../crates/auv-cli-invoke/src/lib.rs) | Current evidence is policy/fixture-oriented, not a blanket app-level live claim. Windows keyboard paths are foreground-only; background policy is explicitly rejected in invoke tests. |
| Foreground application activation | `app.activate` | `tested` | `tested` | `not claimed` | [macOS activation verification tests](../crates/auv-driver-macos/src/application_test.rs), [Windows invoke activation tests](../crates/auv-cli-invoke/src/commands/app_test.rs), [Windows activation driver tests](../crates/auv-driver-windows/src/application_test.rs) | Activation verifies foreground observation when available; it is not semantic app-state success. macOS targets bundle IDs; Windows targets process names. |
